...
注：（8个字节的倍数）对象的内存布局（对象头，实例数据，对齐填充）
...
偏向锁只有在遇到其他线程尝试竞争锁时，持有偏向锁的线程才会释放锁，否则线程是不会主动释放偏向锁；
...
注：批量和禁用是针对类的【阈值也是针对类】；批量重偏向和批量撤销
1. JVM默认加锁是在20次之后，认为对象为偏向锁，直接进行批量偏向，而非单个单个线程偏向，撤销；

2. 对对象超过40次撤销之后，会禁用偏向锁；【同一个类，认为类对应的对象有问题，直接禁用该类偏向锁，升级为轻量级锁（锁中）】


## 锁升级过程
1. 当对象处于无锁状态时，线程进入同步代码块，会判断对象头和栈帧中锁记录是否有当前线程的id，如果没有，则cas操作设置对象头，变为偏向锁，当下次该线程进入同步代码块的时候，不需要进行设置，只需要判断对象头是否有当前线程的偏向锁即可；如果没有或不是，则设置cas设置，升级为轻量级锁；【偏向锁只有在有线程竞争的情况下才会释放锁】
2. 轻量级锁升级，是在线程进入同步代码块之前，在栈帧中创建锁记录，并将对象头设置到锁记录中，然后通过cas将对象头设置为锁记录指针，设置成功当前线程获取锁；否则，表示有其他线程竞争，进行自旋获取锁；
3. 重量级锁升级，当轻量级锁自旋时（默认10次）不能一直自旋，自旋一定次数后会将其升级为重量级锁，并将当前线程挂起阻塞；重量级锁会将竞争的线程阻塞；
