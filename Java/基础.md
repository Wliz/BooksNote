...
## Java Stream流使用注意事项
1. 使用parallelStream并行流时，不能使用Collectors.groupingBy和Collectors.toMap，需要使用Collectors.GroupingByConcurrent和Collectors.toConcurrentMap，原因是通过key来操作合并多个map的时候比较消耗性能（因为存在并行执行）；
2. ParallelStream默认使用的线程池是ForkJoinPool.commonPool();
3. IntStream流可以遍历集合，适用于在遍历基本数据类型的数据（或从对象获取）的情况，否则使用正常的Stream流即可；
...
1. ConcurrentLinkedQueue非阻塞队列

    其中内部类Node适用于垃圾回收环境，采用CAS机制；
